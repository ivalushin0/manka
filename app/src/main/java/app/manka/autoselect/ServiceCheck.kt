...
object ServiceCheck {
    ...
    val GROUPS = listOf("youtube", "instagram", "facebook", "tiktok", "discord", "telegram")
    ...
    suspend fun run(prefs: Prefs, timeoutSec: Int): Map<String, Int> {
        ...
        val results = SiteChecker().check(urls.values.flatten().distinct(), 1, timeoutSec).associateBy { it.url }
